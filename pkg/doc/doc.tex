...
Edith Gabriel, {\tt edith.gabriel@univ-avignon.fr}.
...
%
%\hspace{1cm} b) $x_i=x_{i-1}+x_{inc}, i=1,\dots,{\tt nx}$;
%$y_j=y_{j-1}+y_{inc}, j=1,\dots,{\tt ny}$.

\subsubsection*{Value}

A list containing:

{\tiny $\bullet$} {\tt x, y}: numeric vectors giving the coordinates of
the points of the rectangular grid,

{\tiny $\bullet$} {\tt X, Y}: {\tt nx}$\times${\tt ny} matrix containing
$x$ and $y$ coordinates respectively,

{\tiny $\bullet$} {\tt pts}: index of the grid points belonging to the polygon,

{\tiny $\bullet$} {\tt xinc, yinc}: mesh size of the grid,

{\tiny $\bullet$} {\tt mask}: {\tt nx}$\times${\tt ny} matrix of logicals.
{\tt TRUE} if the grid point belongs to the polygon.
...
\begin{verbatim}
 ...
 polygon(poly1,lwd=2)
 points(as.vector(xygrid$X)[xygrid$pts],as.vector(xygrid$Y)[xygrid$pts],
        pch=19,col=4,cex=0.4)
\end{verbatim}

\clearpage

\subsection{{\tt sim.stpp}}

\subsubsection*{Description}

Generate a spatio-temporal point pattern.
...
\begin{tabular}[c]{rl}
  \verb#class# & Class of process ; must be chosen among "poisson", "cluster",
   "cox", "infectious" \\
   & and "inhibition" \\
   &  and "infectious". \\
  \verb#s.region# & two-column matrix specifying polygonal region containing
  all data locations. \\
  & If \verb#s.region# is missing, the unit square is considered. \\
  \verb#t.region# & vector containing the minimum and maximum values of
  the time interval. \\
  & If \verb#t.region# is missing, the interval [0,1] is considered. \\
  \verb#npoints# & number of points to simulate. If \verb#NULL#, the
  number of points is from a \\
  & Poisson distribution with mean the double integral of \verb#lambda# over \\
  & \verb#s.region# and \verb#t.region#. \\
  \verb#...# & Additional arguments appropriate to the particular class of
   process specified. \\
\end{tabular}
...
{\tiny $\bullet$} \verb#xyt#: matrix (or list of matrices if \verb#nsim#$>1$)
containing the points $(x,y,t)$ of the simulated point process.

{\tiny $\bullet$} \verb#s.region, t.region, lambda#: parameters passed in
argument.

It also return information depending on the class of processes
concerned.
...
\subsubsection*{See also}
 ...
 \verb#animation#, \verb#rinfec#, \verb#rinhib#, \verb#rlgcp#, \verb#rpcp# and \verb#rpp#.
