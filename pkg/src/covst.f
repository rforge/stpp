...
c
      ...
      double precision x,t,param(6),theta(3)
      double precision psit,res
      double precision cauchy,stable,matern
      double precision p1,p2,p3,p4,p5,p6
      ...
      if(p5.eq.1) psit=dsqrt((t**p3+1d0)**(p4))
      if(p5.eq.2) psit=dsqrt((1d0+(t**p3)/p4)/(1d0+(t**p3)))
      if(p5.eq.3) psit=dsqrt(-log(t**p3+1d0/p4)/log(p4))
      ...
      if(p2.eq.3) then
         ...
         theta(2)=1d0
         theta(3)=p1
         res = psit**(-p6) * matern(theta,x/psit)
      end if
